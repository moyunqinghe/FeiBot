...
def test_factory_builds_anthropic_sdk(monkeypatch) -> None:
    captured = {}
    ...
    monkeypatch.setattr("llm_protocols.factory.Anthropic", fake_anthropic)
    ...
    assert isinstance(driver, AnthropicMessagesDriver)
    assert captured["base_url"] == "https://api.anthropic.test"
    assert captured["max_retries"] == 0


def test_factory_adapts_anthropic_v1_api_root_for_sdk(monkeypatch) -> None:
    captured = {}
    ...
    build_protocol_driver(
        protocol="anthropic_messages",
        api_key="secret",
        base_url="https://llm-center.example.test/llm/v1/",
        model="claude-opus-5",
        timeout_seconds=30.0,
    )

    assert captured["base_url"] == "https://llm-center.example.test/llm"


def test_factory_adapts_anthropic_full_messages_endpoint(monkeypatch) -> None:
    captured = {}
    ...
    build_protocol_driver(
        protocol="anthropic_messages",
        api_key="secret",
        base_url="https://llm-center.example.test/llm/v1/messages",
        model="claude-opus-5",
        timeout_seconds=30.0,
    )
